...
aws elasticbeanstalk update-environment --application-name regression-tests --environment-name java-aws-elasticbeanstalk --version-label $now --option-settings Namespace=aws:elasticbeanstalk:application:environment,OptionName=ROOKOUT_TOKEN,Value=$ROOKOUT_TOKEN,OptionName=ROOKOUT_ROOK_TAGS,Value="http://java-aws-elasticbeanstalk",OptionName=ROOKOUT_DEBUG,Value=1 ; \
sleep 40 ; \
