...
aws elasticbeanstalk update-environment --application-name regression-tests --environment-name java-aws-elasticbeanstalk --version-label $now --option-settings Namespace=aws:elasticbeanstalk:application:environment,OptionName=ROOKOUT_TOKEN,Value=$ROOKOUT_TOKEN ; \
sleep 40 ; \
